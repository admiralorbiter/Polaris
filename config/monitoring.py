...
class ImporterMonitoring:
    ...
    DEDUPE_CANDIDATES_TOTAL = (
        Counter(
            "importer_dedupe_candidates_total",
            "Total fuzzy dedupe candidates generated by match type.",
            labelnames=("match_type",),
        )
        if Counter
        else None
    ...
    @classmethod
    def record_fuzzy_candidate(cls, *, match_type: str) -> None:
        label = match_type or "unknown"
        if cls.DEDUPE_CANDIDATES_TOTAL:
            cls.DEDUPE_CANDIDATES_TOTAL.labels(match_type=label).inc()
