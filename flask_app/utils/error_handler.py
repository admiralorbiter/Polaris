...
class ErrorAlertingSystem:
    ...
    def should_send_alert(self, alert_type: str, error_key: str) -> bool:
        ...
        # Clean old entries
        self.error_counts[error_key] = [
            timestamp for timestamp in self.error_counts[error_key] if timestamp > hour_ago
        ]
        ...
        try:
            ...
            if not all([smtp_server, smtp_username, smtp_password, admin_emails]):
                current_app.logger.warning(
                    "Email alerting configured but missing required settings"
                )
                return
            ...
            subject = f"[{severity}] {app_name} Error: {error_type}"
                ...
                "attachments": [
                    {
                        ...
                        "fields": [
                            ...
                            {
                                "title": "Endpoint",
                                "value": context.get("endpoint", "N/A"),
                                "short": True,
                            },
                            {
                                "title": "User ID",
                                "value": str(context.get("user_id", "N/A")),
                                "short": True,
                            },
                            {
                                "title": "IP Address",
                                "value": context.get("ip_address", "N/A"),
                                "short": True,
                            },
                            {
                                "title": "Error Message",
