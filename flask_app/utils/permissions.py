...
def get_user_organizations(user):
    ...
    user_orgs = (
        UserOrganization.query.options(joinedload(UserOrganization.organization))
...
def get_user_role_in_organization(user, organization):
    ...
    user_org = (
        UserOrganization.query.options(joinedload(UserOrganization.role))
...
def require_organization_membership(user, organization):
    ...
    user_org = UserOrganization.query.filter_by(
        user_id=user.id, organization_id=organization.id, is_active=True
    ).first()
...
def can_assign_role(user, target_role, organization=None):
    ...
    user_role_level = role_hierarchy.get(user_role.name, 0)
    target_role_level = role_hierarchy.get(
        target_role.name if isinstance(target_role, Role) else target_role, 0
    )
...
def validate_role_assignment(user, target_role, organization=None):
    ...
    if not can_assign_role(user, target_role, organization):
        role_name = (
            target_role.display_name if hasattr(target_role, "display_name") else str(target_role)
        )
