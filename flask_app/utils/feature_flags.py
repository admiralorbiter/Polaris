...
def set_feature_flag(flag_name, value, organization_id=None, flag_type='boolean', is_system_flag=False):
    """
    ...
    Args:
        flag_name: Name of the feature flag
        value: Value to set
        organization_id: Optional organization ID. If None and not system flag, uses current organization context
    ...
    if org_id is None:
        organization = get_current_organization()
        if not organization:
            current_app.logger.error("Cannot set organization feature flag without organization context")
            return False
