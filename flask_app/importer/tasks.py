...
from flask_app.importer.pipeline.salesforce import ingest_salesforce_sessions as run_salesforce_sessions_ingest
...
from flask_app.importer.pipeline.salesforce_event_loader import LoaderCounters as EventLoaderCounters
...
def ingest_salesforce_accounts(
    ...
    *,
    ...
    """
    Execute the Salesforce Account (Organization) ingest pipeline via the importer worker.
    """
    ...
    try:
        ...
        summary = run_salesforce_accounts_ingest(
            import_run=run,
            extractor=extractor,
            watermark=watermark,
            staging_batch_size=500,
        ...
        # Run DQ validation and clean promotion (same as CSV pipeline)
        dq_summary = run_minimal_dq(run, dry_run=dry_run, csv_rows=None)
        clean_summary = promote_clean_organizations(run, dry_run=dry_run)

        if dry_run:
            counters = OrgLoaderCounters()
            run.status = ImportRunStatus.SUCCEEDED
            run.finished_at = datetime.now(timezone.utc)
            run.metrics_json = run.metrics_json or {}
            run.metrics_json.setdefault("salesforce", {})["max_source_updated_at"] = (
        ...
        core_summary = CoreLoadSummary(
            ...
            rows_missing_external_id=0,  # Salesforce loader doesn't track missing IDs
            rows_soft_deleted=counters.deleted,
            duplicate_emails=(),  # Not applicable for organizations
            dry_run=dry_run,
        )
        ...
        persist_idempotency_summary(
            ...
            clean_summary=clean_summary,
            core_summary=core_summary,
            fuzzy_summary=None,  # Organizations don't use fuzzy dedupe
        )
        current_app.logger.info(
            "Salesforce Account import run completed",
            extra={
                "importer_run_id": run_id,
                "salesforce_job_id": summary.job_id,
                "salesforce_batches_processed": summary.batches_processed,
        ...
        if recovery_run is not None:
            ...
            db.session.commit()
        current_app.logger.exception(
            "Salesforce Account import run failed",
            extra={
                "importer_run_id": run_id,
                "importer_error": str(exc),
            },
...
@shared_task(name="importer.pipeline.ingest_salesforce_sessions", bind=True)
def ingest_salesforce_sessions(
    ...
    *,
    ...
    """
    Execute the Salesforce Session (Event) ingest pipeline via the importer worker.
    """
    ...
    try:
        ...
        summary = run_salesforce_sessions_ingest(
            import_run=run,
            extractor=extractor,
            watermark=watermark,
            staging_batch_size=500,
        ...
        # Run DQ validation and clean promotion (same as CSV pipeline)
        dq_summary = run_minimal_dq(run, dry_run=dry_run, csv_rows=None)
        clean_summary = promote_clean_events(run, dry_run=dry_run)

        if dry_run:
            counters = EventLoaderCounters()
            run.status = ImportRunStatus.SUCCEEDED
            run.finished_at = datetime.now(timezone.utc)
            run.metrics_json = run.metrics_json or {}
            run.metrics_json.setdefault("salesforce", {})["max_source_updated_at"] = (
        ...
        core_summary = CoreLoadSummary(
            ...
            rows_missing_external_id=0,  # Salesforce loader doesn't track missing IDs
            rows_soft_deleted=counters.deleted,
            duplicate_emails=(),  # Not applicable for events
            dry_run=dry_run,
        )
        ...
        persist_idempotency_summary(
            ...
            clean_summary=clean_summary,
            core_summary=core_summary,
            fuzzy_summary=None,  # Events don't use fuzzy dedupe
        )
        current_app.logger.info(
            "Salesforce Session import run completed",
            extra={
                "importer_run_id": run_id,
                "salesforce_job_id": summary.job_id,
                "salesforce_batches_processed": summary.batches_processed,
        ...
        if recovery_run is not None:
            ...
            db.session.commit()
        current_app.logger.exception(
            "Salesforce Session import run failed",
            extra={
                "importer_run_id": run_id,
                "importer_error": str(exc),
            },
