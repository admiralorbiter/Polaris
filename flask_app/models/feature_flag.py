...
class OrganizationFeatureFlag(BaseModel):
    ...
    flag_value = db.Column(db.Text, nullable=False)  # JSON string for complex values, or simple string/bool
    flag_type = db.Column(db.String(20), default='boolean', nullable=False)  # boolean, string, integer, json
    ...
    def get_flag(organization_id, flag_name, default=None):
        ...
        try:
            ...
            current_app.logger.error(f"Database error getting flag {flag_name} for org {organization_id}: {str(e)}")
            return default
    ...
    def set_flag(organization_id, flag_name, value, flag_type='boolean'):
        ...
        try:
            ...
            db.session.rollback()
            current_app.logger.error(f"Database error setting flag {flag_name} for org {organization_id}: {str(e)}")
            return False
