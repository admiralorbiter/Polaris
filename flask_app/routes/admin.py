...
from flask_app.utils.permissions import (
    super_admin_required, permission_required, get_current_organization
)
        ...
        try:
            ...
            if current_user.is_super_admin:
                ...
                if organization:
                    ...
                    org_user_ids = [uo.user_id for uo in UserOrganization.query.filter_by(
            ...
            if current_user.is_super_admin:
                ...
                recent_actions = AdminLog.query.filter_by(
            ...
            if form.validate_on_submit():
                ...
                if not is_super_admin and not current_user.is_super_admin:
                    ...
                    if not org_id or org_id == 0:
                        flash('Organization is required for non-super admin users. Please select an organization from the dropdown.', 'danger')
                        return render_template('admin/create_user.html', form=form, organization=organization)
                ...
                if error:
                    ...
                    if org_id and role_id and not is_super_admin:
                        ...
                        if org and role:
                            ...
                            if not existing:
                                ...
                                db.session.commit()
                                current_app.logger.info(f"Added user {new_user.username} to organization {org.name} with role {role.display_name}")
                        else:
                            current_app.logger.warning(f"Could not add user to organization: org={org_id}, role={role_id}")
                    ...
                    current_app.logger.info(f"User {new_user.username} created successfully, redirecting to admin_users")
    ...
    def admin_view_user(user_id):
        ...
        try:
            ...
            if current_user.is_super_admin:
                ...
                actions = AdminLog.query.filter_by(
                    admin_user_id=current_user.id,
            ...
            if form.validate_on_submit():
                ...
                if error:
                    ...
                    if not is_super_admin and hasattr(form, 'role_id'):
                        ...
                        current_app.logger.debug(f"Edit User - Organization ID from form: {org_id_raw}, converted: {org_id}, Role ID: {role_id}")
