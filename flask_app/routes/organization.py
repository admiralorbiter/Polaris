...
from flask_app.models import Organization, UserOrganization, db, AdminLog
from flask_app.forms import CreateOrganizationForm, UpdateOrganizationForm
from flask_app.utils.permissions import super_admin_required, get_current_organization
...
def register_organization_routes(app):
    ...
    def admin_view_organization(org_id):
        ...
        try:
            ...
            user_org_ids = [uo.user_id for uo in UserOrganization.query.filter_by(
                organization_id=org_id,
                is_active=True
            ).all()]
            active_users = User.query.filter(
                User.id.in_(user_org_ids),
                User.is_active == True
    ...
    def admin_delete_organization(org_id):
        ...
        try:
            ...
            if user_count > 0:
                flash(f'Cannot delete organization {org_name}. It has {user_count} user(s) assigned. Please remove all users first.', 'danger')
