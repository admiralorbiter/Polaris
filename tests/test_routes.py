...
from flask_app.models import AdminLog, Organization, Role, SystemMetrics, User, UserOrganization, db
...
class TestAuthRoutes:
    ...
    def test_login_invalid_username(self, client, app):
        ...
        with app.app_context():
            ...
            assert (
                "Invalid username or password" in response_text
                or "A database error occurred" in response_text
                or "alert-danger" in response_text
            )
    ...
    def test_login_missing_fields(self, client):
        ...
        response_text = response.data.decode("utf-8")
        assert (
            "Password is required" in response_text
            or "alert-danger" in response_text
    ...
    def test_login_empty_fields(self, client):
        ...
        response_text = response.data.decode("utf-8")
        assert (
            "Username is required" in response_text
            or "Password is required" in response_text
            or "alert-danger" in response_text
            or "required" in response_text.lower()
        )
    ...
    def test_login_form_validation(self, client):
        ...
        response_text = response.data.decode("utf-8")
        assert (
            "Username must be between 3 and 64 characters" in response_text
            or "alert-danger" in response_text
            or "required" in response_text.lower()
        )
        ...
        response_text = response.data.decode("utf-8")
        assert (
            "Password must be at least 6 characters long" in response_text
            or "alert-danger" in response_text
            or "required" in response_text.lower()
        )
...
class TestAdminRoutes:
    ...
    def test_admin_dashboard_not_super_admin(self, client, test_user, app):
        ...
        with app.app_context():
            ...
            response_text = response.data.decode("utf-8")
            assert "Polaris" in response_text
