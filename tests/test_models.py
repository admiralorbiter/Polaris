...
class TestUserModel:
    ...
    def test_get_full_name_without_names(self, app):
        ...
        with app.app_context():
            user = User(username="minimaluser", email="minimal@example.com", password_hash="hash")
    ...
    def test_user_default_values(self, app):
        ...
        with app.app_context():
            user = User(username="defaultuser", email="default@example.com", password_hash="hash")
            db.session.add(user)
            db.session.commit()
...
class TestAdminLogModel:
    ...
    def test_admin_log_repr(self, admin_user, app):
        ...
        with app.app_context():
            ...
            log_entry = AdminLog(admin_user_id=admin_user.id, action="TEST_ACTION")
    ...
    def test_log_action_minimal_params(self, admin_user, app):
        ...
        with app.app_context():
            ...
            result = AdminLog.log_action(admin_user_id=admin_user.id, action="LOGIN")
    ...
    def test_log_action_database_error(self, mock_commit, admin_user, app):
        ...
        with app.app_context():
            ...
            result = AdminLog.log_action(admin_user_id=admin_user.id, action="TEST_ACTION")
...
class TestSystemMetricsModel:
    ...
    def test_system_metrics_repr(self, app):
        ...
        with app.app_context():
            metric = SystemMetrics(metric_name="test_metric", metric_value=100.0)
    ...
    def test_get_metric_existing(self, app):
        ...
        with app.app_context():
            metric = SystemMetrics(metric_name="existing_metric", metric_value=75.0)
    ...
    def test_set_metric_new(self, app):
        ...
        with app.app_context():
            result = SystemMetrics.set_metric("new_metric", value=99.9, data='{"status": "active"}')
    ...
    def test_set_metric_update_existing(self, app):
        ...
        with app.app_context():
            ...
            # Update the metric
            result = SystemMetrics.set_metric("update_metric", value=75.0, data='{"new": "data"}')
    ...
    def test_system_metrics_unique_constraint(self, app):
        ...
        with app.app_context():
            metric1 = SystemMetrics(metric_name="unique_metric", metric_value=10.0)
            db.session.add(metric1)
            db.session.commit()

            # Try to create another metric with the same name
            metric2 = SystemMetrics(metric_name="unique_metric", metric_value=20.0)
...
class TestOrganizationModel:
    ...
    def test_organization_creation_minimal_fields(self, app):
        ...
        with app.app_context():
            org = Organization(name="Minimal Org", slug="minimal-org")
            db.session.add(org)
            db.session.commit()
    ...
    def test_organization_unique_slug(self, test_organization, app):
        ...
        with app.app_context():
            duplicate_org = Organization(
                name="Different Name", slug="test-organization"  # Same slug as test_organization
            )
    ...
    def test_organization_safe_create_success(self, app):
        ...
        with app.app_context():
            org, error = Organization.safe_create(
                name="Safe Org", slug="safe-org", description="Safely created", is_active=True
            )
    ...
    def test_organization_safe_create_duplicate_slug(self, test_organization, app):
        ...
        with app.app_context():
            org, error = Organization.safe_create(
                name="Duplicate", slug="test-organization", is_active=True  # Duplicate
            )
    ...
    def test_organization_safe_update_success(self, test_organization, app):
        ...
        with app.app_context():
            # Reattach to session
            org = db.session.get(Organization, test_organization.id)
            result, error = org.safe_update(name="Updated Name", description="Updated description")
...
class TestRoleModel:
    ...
    def test_role_creation_minimal_fields(self, app):
        ...
        with app.app_context():
            role = Role(name="minimal_role", display_name="Minimal Role")
            db.session.add(role)
            db.session.commit()
    ...
    def test_role_relationships_user_organizations(
        ...
        with app.app_context():
            ...
            user_org = UserOrganization(
                user_id=test_user.id, organization_id=test_organization.id, role_id=test_role.id
            )
            db.session.add(user_org)
            db.session.commit()
...
class TestPermissionModel:
    ...
    def test_permission_creation_minimal_fields(self, app):
        ...
        with app.app_context():
            permission = Permission(name="minimal_permission", display_name="Minimal Permission")
            db.session.add(permission)
            db.session.commit()
...
class TestSystemFeatureFlagModel:
    ...
    def test_system_feature_flag_get_value_boolean(self, app):
        ...
        with app.app_context():
            flag = SystemFeatureFlag(flag_name="bool_flag", flag_value="true", flag_type="boolean")
            db.session.add(flag)
            db.session.commit()
    ...
    def test_system_feature_flag_get_value_integer(self, app):
        ...
        with app.app_context():
            flag = SystemFeatureFlag(flag_name="int_flag", flag_value="42", flag_type="integer")
            db.session.add(flag)
            db.session.commit()
    ...
    def test_system_feature_flag_set_value(self, app):
        ...
        with app.app_context():
            flag = SystemFeatureFlag(flag_name="set_flag", flag_value="false", flag_type="boolean")
            flag.set_value(True)
            assert flag.flag_value == "true"
