...
class TestUserRegistrationWorkflow:
    ...
    def test_admin_creates_user_workflow(self, client, admin_user, app):
        ...
        with app.app_context():
            ...
            admin_log = AdminLog.query.filter_by(
                admin_user_id=admin_user.id, action="CREATE_USER", target_user_id=new_user.id
            ).first()
    ...
    def test_user_login_after_creation_workflow(self, client, app):
        ...
        with app.app_context():
            ...
            login_response = client.post(
                "/login",
                data={"username": "createduser", "password": "userpass123"},
                follow_redirects=True,
            )
            assert login_response.status_code == 200
            ...
            assert b"Polaris" in admin_response.data
    ...
    def test_user_management_complete_workflow(self, client, admin_user, app):
        ...
        with app.app_context():
            ...
            delete_response = client.post(
                f"/admin/users/{test_user.id}/delete", follow_redirects=True
            )
...
class TestAuthenticationWorkflow:
    ...
    def test_inactive_user_login_workflow(self, client, app):
        ...
        with app.app_context():
            ...
            login_response = client.post(
                "/login", data={"username": "inactiveuser", "password": "userpass123"}
            )
...
class TestAdminWorkflow:
    ...
    def test_admin_dashboard_workflow(self, client, admin_user, app):
        ...
        with app.app_context():
            ...
            assert (
                b"total_users" in dashboard_response.data
                or b"users" in dashboard_response.data.lower()
            )
    ...
    def test_admin_user_creation_validation_workflow(self, client, admin_user, app):
        ...
        with app.app_context():
            ...
            assert (
                b"Username already exists" in create_response.data
                or b"already exists" in create_response.data.lower()
                or b"error" in create_response.data.lower()
            )
...
class TestDataIntegrityWorkflow:
    ...
    def test_admin_log_integrity_workflow(self, client, admin_user, app):
        ...
        with app.app_context():
            ...
            db.session.expire_all()
            admin_log = AdminLog.query.filter_by(
                admin_user_id=admin_user.id, action="CREATE_USER"
            ).first()
            assert admin_log is not None
            ...
            assert (
                "CREATE_USER" in response_text
                or "Create User" in response_text
                or "Created user" in response_text
            )
...
class TestMultiTenancyWorkflows:
    ...
    def test_user_multiple_organizations(self, client, test_user, app):
        ...
        with app.app_context():
            ...
            user_org1 = UserOrganization(
                user_id=test_user.id, organization_id=org1.id, role_id=role1.id
            )
            user_org2 = UserOrganization(
                user_id=test_user.id, organization_id=org2.id, role_id=role2.id
            )
    ...
    def test_user_assignment_to_organization_workflow(
    ...
    def test_permission_check_across_organizations(self, client, test_user, app):
        ...
        with app.app_context():
            ...
            permission = Permission(
                name="view_volunteers", display_name="View Volunteers", category="volunteers"
            )
            ...
            user_org = UserOrganization(
                user_id=test_user.id, organization_id=org1.id, role_id=role.id
            )
