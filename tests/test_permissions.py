...
from flask_app.models import Organization, Permission, Role, RolePermission, UserOrganization, db
...
class TestPermissionHelperFunctions:
    ...
    def test_get_user_organizations_regular_user(
    ...
    def test_get_user_organizations_excludes_inactive(
        self, test_user, test_organization, test_organization_inactive, test_role, app
    ):
        ...
        with app.app_context():
            ...
            user_org2 = UserOrganization(
                user_id=test_user.id, organization_id=inactive_org_id, role_id=test_role.id
            )
    ...
    def test_get_user_role_in_organization_super_admin(
        self, super_admin_user, test_organization, app
    ):
    ...
    def test_get_user_role_in_organization_regular_user(
        self, test_user, test_organization, test_role, app
    ):
    ...
    def test_has_permission_with_organization(
        self, test_user, test_organization, test_role, test_permission, app
    ):
    ...
    def test_has_permission_without_organization(
        self, test_user, test_organization, test_role, test_permission, app
    ):
    ...
    def test_get_available_organizations_super_admin(
        self, super_admin_user, test_organization, app
    ):
    ...
    def test_get_available_organizations_regular_user(
        self, test_user, test_organization, test_role, app
    ):
    ...
    def test_can_assign_role_hierarchy(self, test_user, test_organization, app):
        ...
        with app.app_context():
            ...
            user_org = UserOrganization(
                user_id=test_user.id, organization_id=org_id, role_id=org_admin.id
            )
            db.session.add(user_org)
            db.session.commit()
    ...
    def test_require_organization_membership_super_admin(
        self, super_admin_user, test_organization, app
    ):
    ...
    def test_require_organization_membership_member(
        self, test_user, test_organization, test_role, app
    ):
...
class TestPermissionDecorators:
    ...
    def test_super_admin_required_blocks_regular_user(self, client, test_user, app):
        ...
        with app.app_context():
            ...
            # Check that we're redirected to index page (Polaris branding)
            # Admin routes redirect non-admin users to index page
            assert b"Polaris" in response.data

    def test_permission_required_with_org_context(
        self, client, test_user, test_organization, test_role, test_permission, app
    ):
